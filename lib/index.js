      ...
      Object.keys(app._pathCache).forEach(function (p) {
        ...
        paths.forEach(function (path) {
          switch (path.op) {
            case 'set':
              if (hasSet) {
                var err = new Error('cannot set path twice `' + p + '`');
                err.path = p;
            ...
            case 'push':
              if (hasMerge) {
                var _err3 = new Error('cannot push and merge to same path `' + p + '`');
                _err3.path = p;
          ...
          if (pointerValue && typeof app._pathCache[pointerValue] === 'undefined') {
            var _err5 = new Error('cannot point to undefined path `' + pointerValue + '`');
            _err5.path = p;
            _err5.paths = app._pathCache[p];
            _err5.pointer = pointerValue;
            throw _err5;
          }
        });
